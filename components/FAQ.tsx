...
import {
  ...
  getPublicPageEditorial,
  type PageEditorialLocale,
} from '@/lib/pageEditorialContent'

const FAQ = ({ locale = 'tr' }: { locale?: PageEditorialLocale }) => {
  const cmsPage = useCmsPage()
  const [openQuestion, setOpenQuestion] = useState<number | null>(null)

  const initialDocument = useMemo(
    () => getPageEditorialFromCmsPage('home', cmsPage?.pageKey === 'home' ? cmsPage : null),
    [cmsPage],
  )
  const [document, setDocument] = useState(initialDocument)

  useEffect(() => {
    if (cmsPage?.pageKey === 'home') {
      setDocument(initialDocument)
      return
    }
    let isMounted = true
    getPublicPageEditorial('home').then((nextDocument) => {
      if (isMounted) setDocument(nextDocument)
    })
    return () => {
      isMounted = false
    }
  }, [cmsPage?.pageKey, initialDocument])

  const content = getFaqEditorialLocale(document, locale)
  ...
  return (
    <section className="py-20 lg:py-28 bg-gradient-to-b from-gray-950 via-black to-gray-950 relative overflow-hidden" lang={locale} data-page-editorial="home-faq">
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        ...
        >
          <div className="inline-block mb-4">
            <div className="flex items-center space-x-3 text-xs text-gray-500">
              ...
              <span className="uppercase tracking-[0.2em] font-light">{content.faq.eyebrow}</span>
              <span className="w-12 h-[1px] bg-gray-700"></span>
            </div>
          </div>
        ...
        >
          <p className="text-gray-400 mb-6">{content.faq.cta.title}</p>
          {content.faq.cta.description && (
            <p className="text-gray-400 mb-6 max-w-2xl mx-auto font-light">{content.faq.cta.description}</p>
          )}
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              ...
              <span className="relative z-10 font-medium">{content.faq.cta.whatsappLabel}</span>
            ...
            >
              ...
              <span>{content.faq.cta.callLabel}</span>
            </a>
          </div>
        </motion.div>
      </div>
